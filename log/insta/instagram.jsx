...
const load = async () => {
  try {
    const result = await window.storage.get("best-days-v1");
...
try { await window.storage.set(“best-days-v1”, JSON.stringify(list)); } catch (_) {}
};
...
noPhotoIcon: { fontSize: "48px", lineHeight: 1 },
...
// counter
counter: { position: "absolute", top: "20px", left: "50%", transform: "translateX(-50%)", fontSize: "11px", letterSpacing: "2px", color: "rgba(237,232,223,0.5)", textTransform: "uppercase", zIndex: 5 },

// bottom actions
actions: { position: "absolute", bottom: "28px", left: "28px", right: "28px", display: "flex", gap: "10px", zIndex: 5 },
btnGhost: { flex: 1, padding: "12px", borderRadius: "100px", border: "1px solid rgba(237,232,223,0.2)", background: "rgba(0,0,0,0.35)", backdropFilter: "blur(10px)", color: "#EDE8DF", fontSize: "13px", cursor: "pointer", letterSpacing: "0.3px" },
btnGold: { flex: 2, padding: "12px", borderRadius: "100px", border: "none", background: "#C8A87A", color: "#0D0C0B", fontSize: "13px", fontWeight: "600", cursor: "pointer", letterSpacing: "0.3px" },
...
<div style={s.root}>
<div style={s.empty}>
<div style={s.emptyTitle}>the best days<br />of your life</div>
...
```
    <div style={s.card}>
      {entry.photo
        ...
        : <div style={s.noPhoto}><div style={s.noPhotoIcon}>◈</div><div style={{ fontSize: "12px" }}>no photo</div></div>}
